			...
			validation(value, function (err, isValid, validationError) {
				if (err) {
					if (err instanceof Schema.ValidationError) {
						err = new Schema.ValidationResultError(err.rule, err.ruleParams, value, err.index, path);
					}
					done(err);
					return;
				}
				...
				done(new Schema.ValidationResultError(validationError.rule, validationError.ruleParams, value, null, path));
			});
		}, done);
	},
 ...
 */
Schema.ValidationError = function ValidationError (rule, ruleParams, index) {
	if (!(this instanceof Schema.ValidationError)) {
		return new Schema.ValidationError(rule, ruleParams, index);
	}
 ...
 */
Schema.ValidationResultError = function ValidationResultError (rule, ruleParams, value, index, path) {
	if (!(this instanceof Schema.ValidationResultError)) {
		return new Schema.ValidationResultError(rule, ruleParams, value, index, path);
	}
	...
	Schema.ValidationError.call(this, rule, ruleParams);
