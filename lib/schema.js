...
var schemaLoader = require('./loader');
...
Schema.prototype = {
	...
	field: function (fieldName) {
		...
		var schema = new this.constructor();
	...
	verify: function (options, value, done) {
		if (done == null) {
			done = value;
			value = options;
			options = null;
		...
		if (!this.hasOwnProperty('_verifier')) {
			this._verifier = this.verifier(options);
		}

		return this._verifier(value, done);
	},
...
SchemaVerifier.prototype = {
	...
	_verifySchema: function (schema, value, path, done) {
		...
		schema.verifierObject.verify(value, function (err) {
			...
			if (schema.isArray) {
				iterate.each(value, function (obj, index, done) {
	...
	_verifyObject: function (schema, value, path, done) {
		...
		iterate.each(schema.fields, function (schema, key, done) {
