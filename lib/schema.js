			...
			validation(value, function (err, isValid, validationError) {
				if (err) {
					if (err instanceof Schema.ValidationError) {
						err = new Schema.ValidationResultError(err.ruleName, err.ruleParams, value, err.index, path);
					}
					done(err);
					return;
				}
 ...
 */
Schema.ValidationError = function ValidationError (ruleName, ruleParams, index) {
	if (!(this instanceof Schema.ValidationError)) {
		return new Schema.ValidationError(ruleName, ruleParams, index);
	}
 ...
 */
Schema.ValidationResultError = function ValidationResultError (ruleName, ruleParams, value, index, path) {
	if (!(this instanceof Schema.ValidationResultError)) {
		return new Schema.ValidationResultError(ruleName, ruleParams, value, index, path);
	}
